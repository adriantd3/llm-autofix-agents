...
- [x] B1 `validation/models.py`: `Literal["CORRECT", "PLAUSIBLE", "OVERFITTING", "FAIL"]`; eliminar `infra_fail_detected`
- [x] B2 `validation/prompt.py`: system prompt con nuevos veredictos + nota de contexto
- [x] B3 `validation/runner.py`: `_query_run_ids()` filtra `final_status = 'success'`; errores de pipeline o runs no evaluables → persistir `FAIL`; eliminar `infra_fail_detected` de `_build_record`
...
- [x] D2 `tests/unit/.../test_validation_schema.py`: cambiar casos con `INFRA_FAIL` → `FAIL`/`OVERFITTING`
