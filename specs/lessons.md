...
## 2026-05-14 (sobreajuste a los tests: parches plausibles pero incompletos)

- **Contexto:** Tras ejecutar el benchmark de BugsInPy con `qwen3.5:9b` + `multi_agent_orchestrator`,
  8 de 9 bugs pasaron los tests. Al comparar los patches generados con los fixes canónicos del repo
  oficial de BugsInPy, se detectó un patrón sistemático: algunos fixes pasaban los tests pero eran
  semánticamente distintos o incompletos respecto a la solución real del desarrollador.
  Casos concretos:
  - `fastapi-1`: el agente solo modificó `encoders.py`; el fix canónico toca 4 archivos
    (applications.py, routing.py, encoders.py, openapi/utils.py) para propagar el parámetro.
  - `cookiecutter-4`: el agente añadió `FailedHookException` y la lógica en `hooks.py`, pero no
    actualizó `generate.py` para capturar la excepción y hacer cleanup. El test no lo ejercitaba.
  - `thefuck-5`: el agente encontró una cadena distinta en el output mockeado del test para
    discriminar el caso; el fix canónico apunta a la cadena semánticamente correcta.
  - `thefuck-7`: el agente cambió la condición pero omitió eliminar `requires_output = False`
    porque ningún test lo validaba.

- **Anti-patrón detectado:** El agente define "root cause" como "lo que hace fallar el test", no
  como "lo que el módulo hace mal". Cuando los tests son un oráculo parcial (cubren solo parte del
  comportamiento), el agente produce un **parche plausible** — correcto para el test, incompleto
  para el sistema. Es el problema clásico de APR: *plausible but not correct patches*.
  El agente usa el test como especificación completa del fix, cuando en realidad es solo evidencia
  de dónde se manifiesta el bug.

- **Qué no hay que hacer:** Asumir que "tests pasan = bug resuelto". No hay ninguna instrucción que
  diferencie explícitamente el oráculo de tests de la especificación completa del comportamiento.
  Las instrucciones anteriores decían "fix the root cause" y "make tests pass" como sinónimos.

- **Por qué estuvo mal:** El agente no tiene incentivo para leer código más allá de lo que ejercita
  el test. Si el test no cubre callers, archivos relacionados, o la otra cara de una interfaz, el
  agente no los toca. El orden mental era: test failure → localizar código → aplicar fix → validar
...
- **Soluciones aplicadas** (en los 5 archivos de instrucciones: `mono_agent.py`, `orchestrator.py`,
  ...
  1. **Code-first diagnosis** (todas las arquitecturas): Se añadió el principio explícito de leer
     la función objetivo en el código fuente *antes* de analizar las assertions del test.
     *"The failing test is evidence of where the bug manifests, not the specification of what to fix.
     The test tells you the symptom; the source code tells you the correct behavior."*

  2. **Completeness check** (mono, patcher, executor, orchestrator): Se añadió un paso obligatorio
     tras aplicar el fix para verificar propagación: si se modificó una firma, se añadió un
     parámetro, o se creó una excepción nueva, buscar callers y archivos relacionados que necesiten
     actualización consistente.
     *"A fix that only touches one side of an interface is incomplete."*

  3. **Desacople test-passing / bug-fixed** (planner + localizer): Se reemplazó la instrucción
     "formulate a repair plan that satisfies ALL test constraints" por una que exige validar que el
     plan tiene sentido para el contrato de la función, no solo para las assertions.
     *"Test constraints are necessary but not sufficient — verify your plan makes semantic sense for
     the function's contract, not just that it makes assertions pass."*

- **Regla preventiva para futuras specs:** Al evaluar resultados de un batch, no clasificar
  automáticamente como "resuelto" todo bug con `status=success`. Revisar el patch generado contra
  el fix canónico para distinguir entre corrección semántica real y sobreajuste al test. En
  benchmarks de calidad, mantener dos métricas separadas: *test pass rate* y *canonical match rate*.
...
## 2026-05-23 (context engineering en planner-executor con modelos thinking)
- Contexto: 4 runs batch con `gemma4-26b-ctx32k` sobre bugs hard de BugsInPy (tornado-6, ansible-1, ansible-2, scrapy-33). Resultado inicial 1/4. Tras análisis de trazas y dos fixes de instrucciones: 3/4.
- Anti-patrón 1 — contrato roto en instrucciones: `PLANNER_INSTRUCTIONS` describía "llama a `transfer_to_executor`" pero ese tool no existe en el planner agent. `PhasedIterationStrategy` usa texto libre como output, no handoffs. El modelo obedece la instrucción, no encuentra el tool, y produce content vacío (0 tokens, ~2-4s). Ocurrió porque las instrucciones copiaron un patrón de la arquitectura handoff que nunca fue implementado aquí.
- Anti-patrón 2 — instrucción FORBIDDEN incorrecta para el rol: `PLANNER_INSTRUCTIONS` tenía "FORBIDDEN: Producing the final iteration record". Esa restricción es para el executor. Para el planner, producir su salida ES la tarea. Instrucción contradictoria con el propósito del agente.
- Anti-patrón 3 — ausencia de "first response must be tool call" en el planner: para bugs con fix obvio en el stack trace, el modelo (thinking mode) razonaba la solución internamente y la intentaba escribir directamente como primera respuesta → content vacío (thinking tokens consumidos, 0 content tokens). El executor ya tenía esta restricción; el planner no.
- Qué no hay que hacer: deshabilitar thinking mode (`options: {think: false}`) como workaround. Probado en run 2: causó regresión en ansible-2 (era success, pasó a fail) porque el modelo necesita razonamiento profundo para bugs de comparación de versiones. La mejora real viene de corregir los contratos de instrucciones.
- Alternativa recomendada: (1) auditar instrucciones de cada agente contra su toolset real — si la instrucción menciona un tool name concreto, verificar que existe. (2) añadir "Your FIRST response MUST be a tool call" a cualquier agente con thinking mode que deba investigar antes de responder. (3) verificar que FORBIDDEN y restricciones de rol son coherentes con la función del agente (planner ≠ executor).
- Impacto cuantitativo: 1/4 → 3/4 con dos líneas de instrucciones modificadas. tornado-6 pasó de 1353s (fallido) a 1439s (exitoso). ansible-1 pasó de morir en 30s (0 tool calls) a resolver en 292s (investigación completa + fix).
- Regla preventiva: al crear un nuevo agente en una arquitectura multi-agente, hacer un checklist: (a) ¿cada tool name mencionado en instrucciones existe en el toolset real?, (b) ¿las FORBIDDEN rules tienen sentido para el ROL de este agente?, (c) ¿se fuerza la primera tool call si el agente debe investigar antes de responder?
