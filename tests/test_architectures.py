...
class MultiAgentOrchestratorArchitectureTests(unittest.TestCase):
    def test_orchestrator_architecture_wires_task_agents_and_tools(self) -> None:
        ...
        tools_by_profile = {
            ...
            "test_runner": [_tool("run_test_target"), _tool("read_file")],
            "orchestrator_main": [
                _tool("search_files"),
                _tool("read_file"),
        ...
        self.assertEqual(architecture.tool_count, 5)  # 3 main tools + 2 sub-agent tools
        self.assertEqual(facade_agent.name, "orchestrator")

        # Two sub-agents: explorer and test_runner
        self.assertEqual(len(architecture.sub_agents), 2)
        ...
        self.assertEqual(architecture.sub_agents[1].agent_name, "test_runner")
        self.assertEqual(architecture.sub_agents[1].agent_role, "test_runner")
        self.assertEqual(architecture.sub_agents[1].tool_profile, "test_runner")

        # build_apr_tools is called once during construction (tool_count) then 3 times inside facade_agent_builder
        ...
        self.assertIn("test_runner", profile_calls)

        # Agents are built in order: explorer, test_runner, orchestrator
        self.assertEqual(
            [call["name"] for call in build_agent_calls],
            ["explorer", "test_runner", "orchestrator"],
        )
        ...
        # Two sub-agent tools: explore_code and run_tests
